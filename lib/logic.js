class FrameOBJ {
    constructor(
        ...
        this.flagCurrentOVBaccumulation = flagCurrentOVBaccumulation;
    }
        ...
        this.stratCurrentOVB.forEach(obj => {
            ...
            if (price >= this.MIN && price <= this.MAX) {
                if (obj.orderSide === "ask") {
                    this.frame[price].selllo = this.flagCurrentOVBaccumulation ? obj.amount : this.frame[price].selllo + obj.amount
                } else {
                    this.frame[price].buylo  = this.flagCurrentOVBaccumulation ? obj.amount : this.frame[price].buylo + obj.amount
                }
            }
        });
...
class SelectDates {
    ...
    findClosest(dataObjByDateKeys, currentTime) {
        ...
        Object.keys(dataObjByDateKeys).forEach(key => {
            if (!dataObjByDateKeys.timestamp) { //first iteration when obj is empty.
                dataObjByDateKeys.timestamp = parseFloat(key);
                dataObjByDateKeys.delta     = Math.abs(parseFloat(key) - currentTime);
            }
            const prevDelta = dataObjByDateKeys.delta;
            const newDelta = Math.abs(parseFloat(key) - currentTime);
            if (newDelta < prevDelta) {
                dataObjByDateKeys.timestamp = key;
                dataObjByDateKeys.delta     = newDelta;
            } 
        })

        return dataObjByDateKeys[`${dataObjByDateKeys.timestamp}`]
    }
